...
- 데이터 구조의 특정 값을 확인한 후 똑같은 동작을 수행하는 코드가곳곳에 등장하는 경우가 있다.
  - 코드베이스에서 특정 값에 대해 똑같이 반응하는 코드가 여러 곳이라면 그 반응들을 한곳으로 모으는게 효율적이다.
- 특수한 경우 공통 동작을 요소하나에 모아 사용하는 것을 `특이 케이스 패턴`이라 부른다
  - 특이 케이스는 여러 형태로 표현할 수 있다. 특이 케이스는 객체에서 단순히 데이터를 읽기만 한다면 반환할 리터럴 객체 형태로 준비하면 된다.
    - 특이 케이스 객체는 이를 캡슐화한 클래스가 반환되도록 만들 수도 있고, 반환을 거쳐 데이터 구조에 추가 시킬 수도 있다.
  - 특히 널은 특이 케이스로 처리할 떄가 많다. 널 외의 다른 특이케이스도 많지만 널 객체가 특이 케이스의 특수한 예라 보는게 맞다.
- 예시
  - [링크](https://github.com/edel1212/refactoring/blob/main/refactoringSkills/conditional/ex02.js)

### 어서션 추가하기

- 예를 들어 제곱근 계산은 입력이 양수일 떄만 정상 동작한다 이러한 경우 `어서션`을 사용하면 코드를 쉽게 파악할 수 있게 도와준다.
- 어서션은 항상 참이라고 가정하는 조건부 문장으로 어서션이 실패했다는건 프로그래머가 잘못 했다는 증거이다.
- 어서션 실패는 시스템의 다른 부분에서는 절대로 검사하지 않아야 하며 어서션이 있고 없고가 프로그램 기능 동작에 영향을 주면 안된다.
- 어서션을 오류 찾기에만 활용하는 사람이 있는데 꼭 `true`가 나와야하는 로직이 있다면 어서션을 코드에 삽입하여 사용하는것도 좋은 방법이 될 수 있다.
  - 단 단위 테스트를 꾸준히 추가하여 사각을 좁혀 개발을 진행 했다면 어서션이 필요가 없는 경우가 많다.
  - 프로그래머가 일으킬만한 오류에만 어선셜을 사용해주자
- 어서션은 소통 측면에서도 충분히 매력적으로 사용할 수있다.
- 예시
...
- 제어 플래그란 코드의 동작을 변경하는데 사용되느 변수를 말 한다.
- 여기서 악취나는 코드는 어딘가에서 값을 계산해 제어플래그에 설정한 후 다른 어딘가의 조건문에서 검사하는 형태로 쓰이는 형식이다.
  - 충분히 리팩터링으로 간소화할 수 있음에도 복잡하게 작성된 코드 그대로 납두는 경우가 많기 때문이다.
- 제어 플래그의 주 서식지
  - 반복문
    - break , continue를 활용해 해결할 수 있지만 그렇지 않기 때문이다.
  - 함수의 return문을 무조건 하나로 하려는사람
    - 함수에서 할일을 다 마쳤다면 return 문을 사용해서 명확하게 알리는 편이 훨씬 효율적이고 직관적이다.
- 예시
  ...
  function good() {
    ...
    // ⭐ 내장 함수 활용
    function good2() {
      // 함수가 참 같은 값을 반환할 때까지 배열 안에 있는 각각의 요소마다 한 번씩 호출합니다.
      if (peole.some((itme) => ["조커", "사루만"].includes(item))) sendAlert();
    }
  }
  ```
...
- API는 모듈과 함수들을 끼워맞추는 연결부이다.
- 이해하기 쉽고 사용허가 쉬운 API를 구한혀는 일은 굉장히 중요하며 동시에 어렵기도 하다 그렇기에 API를 개선하는 방법을 새로 깨달을 때마다 리팩터링 해주자

### 질의 함수와 변경 함수 분리하기

- 좋은 코드는 외부에서 관찰 할 수 있는 `겉보기 부수효가가 전혀 없이` 값을 반환 해주는 함수를 추구해야한다.
  - 요청하는 함수 내부에서 목적 반환 외 다른 기능이 실행 실행 되지 않게 끔 하는 것
- 겉보기 부수효과가 전혀 없다면 이용할 떄 신경 쓸 거리가 매우 적어지기 떄문에 관리에도 용의하다.
- 질의함수는 모두 부수 효과를 없애야 한다는 규칙을 따르면 좋은데, 이를 `명령 - 질의 분리` 라는 규칙으로도 불린다.
  - 책의 저자 또한 "무조건 해당 규칙을 따라야한다"까지는 아니지만 되도록 따르려 노력한다 하며 그 효과 또한 톡톡히 봤다 한다.
- 흔히 사용하는 최적화 기법 중 요청된 값을 캐시 해두고 다음번 호출때 빠르게 응답하는 방법이 있는게, 이러한 캐싱도 객체의 상태를  
  변경 하지만 객체 밖에서는 관찰 할 수 없다
  - 즉 겉보기 부수효과 없이 어떤 순서로 호출하든 모든 호출에 항상 똑같은 값을 반환할 뿐이다.
- 예시

  ```javascript
  // 배열 내 악당을 찾으면 알람을 띄워 줌
  const alertForMiscrenant = (people) => {
    for (const p of people) {
      if (p === "조커") {
        setOffAlarms();
        return p;
  ...
  // 👉 의도에 맞게 함수변 변경 및 serOffAlarms()제거
...
- 플래스인수란 호출되는 함수가 실행할 로직을 호출하는 쪽에서 선택하기 위해 전달하는 인수이다.
  - 쉽게 설명하면 `bookConcert(aCustomer, ture);` // true면 프리미엄, false면 일반
  - 꼭 Boolean 타입이 아니여도 열거형 혹은 문자열로 함수를 나눠서 사용하는것고 플래그 인수에 포함된다.
- 플래그함수가 안 좋은 이유는 호출할 수 있는 함수들이 무엇이고 어떻게 호출해야 하는지를 이해하기 어려워지기 떄문이다.
  - 플래그 함수가 있으면 함수들의 기능 차이가 잘 드러나지 않기 떄문이다.
    - 함수를 사용할떄 인수로 어떤 값을 넘겨야 하는지 또 알아내야하는 번거러움이 있다.
    - 여기에 Boolean 타입일 경우 이러한 문제는 더욱 심각해진다 .. 뭐가 true일 때 실행되는지 코드를 까봐야 알 수 있음..
- 그렇기에 특정한 기능 하나만 수행하는 명시적 함수를 제공하는 편이 훨씬 깔끔하다.
  - 플래그 인수를 제거함녀 코드가 깔끔해짐은 물론이고 프로그래밍 도구에도 도움을 준다.
- 가끔씩 플래그 함수 없이 구현할 수 없을거 같아 구현된 코드가 있을 수 있다. 가능한 조합 함수를 만들어야하는 문제라 생각해서...  
  하지만 이러한 문제는 플래그 인수로 해결할게 아닌 함수 하나가 너무 많은 일을 처리하고 있다는 신호로 보고 함수를 더 간단하게 만들어 보자
- 예시

  ```javascript
  // 💩 deliveryDate() 함수가 어떤것을 의미하는지 알 수가 없음
  aShipment.deliverDate = deliveryDate(anOrder, ture);
  aShipment.deliverDate = deliveryDate(anOrder, false);
  ...
  /** 플래그인수 제거하기 👍 **/
  aShipment.deliverDate = rushDeliveryDate(anOrder);
  aShipment.deliverDate = regularDeliverDate(anOrder);

  const rushDeliveryDate = (anOrder) => {
    /** Logic */
  };
  const regularDeliverDate = (anOrder) => {
    /** Logic */
  };
...
- 하나의 레코드에서 값 두어 개를 가져와 인수로 넘기는 코드 보다는 그 값들 대신 레코드를 통째로`(오프젝트형태)` 너믹고 함수 본문에서 꺼내 스도록 한다.
  - 레코드를 통째로 넘기면 변화에 대응하기가 더 쉬워진다. 갑자기 필요한 데이터가 늘어날 겅여 함수 내부에서 꺼내 쓰면 됨
- 하지만 함수가 레코드 자체에 의존하기를 원치 않을 떄는 해당 리팩터링을 수행하지 않으면된다.
  - 레코드와 함수가 서로 다른 모듈에 속한 상황이라면 특히 더 그렇다.
- 어떤 객체로부터 값 몇개를 얻은 후 "그 값들로만" 처리하는 로직이 있다면 건 매개변수로 받아 사용이 아닌 Class로 만들어 객체로 사용되어야 한다.
- 한 객체가 제공하는 기능 중 항상 똑같은 일부만을 사용하는 코드가 많다며면, 그 기능만 따로 묶어서 Class로 추출하라는 신호다
- 예시

  ```javascript
  const low = aRoom.daysTempRange.low;
  const high = aRoom.daysTempRage.high;
  if(!aPlan.withinRange(low, high)) alert("방의 온도가 지정 범위를 벗어났습니다.")
  ...
  // 💯 값을 객체 통쨰로 넘겨줌
  if(!aPlan.withinRange(aRoom.daysTempRange)) alert("방의 온도가 지정 범위를 벗어났습니다.")
  ...
  class HeatingPaln{
   ...
   /** 객체 통째로 넘기기 - 새함수로 만들어 다른방식으로 사용 👍 **/

  // ⭐ 조금더 코드를 나눠놔서 직관적으로 해석이 가능해 짐
  class HeatingPaln{
...
- 매개변수 목록은 함수의 변동 요인을 모아놓은 곳이다.
  - 매개변수 목록은 중복은 피하는게 좋고 또한 짧을 수록 이해하기가 쉽다.
- 매개변수를 제거하면 값을 결정하는 책임 주체가 달라진다.
  - 매개변수가 있다면? : 결졍 주체가 호출자
  - 매개변수가 없다면? : 결정 주체가 피호출 함수가 된다.
- 저자는 호출하는 쪽을 감소화하는 쪽을 선호하여(매개변수를 없애는 방향) 간소화 하게 만든다.
  - 단 피호출 함수가 그 역할을 수행하기에 적합할 때만 그렇게 한다
    - 매개 변수를 제거하면 피호출 함수에 원치 않은 의존성이 생길 경우에 사용할 수 없기 떄문이다.
- 주의사항
  - 대상 함수가 참조투명해야 한다는 것이다. 참조 투명이란 "함수에 똑같은 값을 건네 호출하면 항상 똑같이 동작한다"는 것이다.
    - 쉽게 설명하면 : 매개변수를 없애는 대신에 가변 전역 변수를 이용하면 안된다는 뜻이기도 하다. -> 전역변수는 위험해!!
- 예시
  ...
  class Order {
    get finalPrice() {
      ...
      const disconuntLevel = this.quantity > 100 ? 2 : 1;
  ...
  class Order {
    ...
    discountedPrice(basePrice) {
      // 🍎 this.discountLevel 내부 함수 호출 후 값을 불러옴
...
- 반대의 리팩터링 : `매개변수를 질의 함수로 바꾸기`
- 함수안에 두기엔 거북한 참조를 발견할 때가 있다.
  - 전역 변수를 참조한다거나 제거하길 원하는 원소를 참조하는 경우가 여기에 속한다.
- 위와 같은 문제는 참조변수를 매개변수로 바꿔서 해결이 가능하다.
  - 이전에 설명했던 `매개변수를 질의 함수로 바꾸기`를 원복해주면 된다.
- 해당 리팩토링은 문제가 있는데 그것은 바로 참조 투명성을 잃게 된다는 것이다. 따라서 어떤 값을 제공할지 호출자가 알아내야 하며  
  결국 호출자가 복잡해지는데 이문제는 결국 책임 소재를 프로그램의 어디에 배정하느냐의 문제로 귀결디며 답을 찾기 쉽지 않으며 항상 정답이  
  있는게 아니기에 프로젝트를 진행하면서 균형접이 이리저리 옮겨질 수 있으니 상황에 맞는 리팩터링을 사용하는 것이 중요하다.
- 예시

  ```javascript
  class HeatingPlan {
    get tragetTemperature() {
      if (thermostat.selectedTemperature > this._max) return this._max;
      else if (thermostat.selectedTemperature < this._min) return this._min;
      else return thermostat.seletedTemperature;
    }
  }
  ...
  /** 질의 함수를 매개변수로 바꾸기 👍 **/
  class HeatingPlan {
    tragetTemperature(selectedTemperature) {
      if (thermostat.selectedTemperature > this._max) return this._max;
      else if (thermostat.selectedTemperature < this._min) return this._min;
      else return thermostat.seletedTemperature;
    }
  ...
  // 호출자  -- 호출부분에서 처리함  ⭐thermostat 값 자체가 전역 변수 이기 떄문에 이게 더 맞는 방법이다!!
  let thermostat = {
    /** 전역 변수임 ㅎㅎ */
  };
  const thePaln = new HeatingPlan();
  thePaln.tragetTemperature(thermostat.selectedTemperature);
  ```

### 세터 제거하기

- 세터가 있다는 것은 필드가 수정될 수 있다는 뜻이다.
- 객체 생성 후 수정되지 않길 원하는 필드라면 세터를 제공하지 않으면 된다.
  - 이러한 효과는 수정하지 않겠다는 의도가 명백해지고 변경경될 가능성 자체가 차단된다.

### 생성자를 팩터리 함수로 바꾸기

- 객체 지향언어에서 제공하는 생성자는 객체를 초기화하는 특별한 용도의 함수이다.
  - 실제로 새로운 객체를 생성할 때면 주로 생성자를 호출한다.
- 하지만 이런 생성자 메서드에도 단점이 있다.
  - 자바의 경우 생성자를 정의한 클래스의 인스턴스르 반환해야한다.
  - new ClassName()을 사용해야 한다.
    - ClassName이 강제 되기에 더 직관적인 이름의 함수로 불러서 사용이 불가능하다.
- 팩터리 함수에는 이런 제약이 없다. 원하는 함수명을 만들고 그에 맞게 인스턴스를 반환하면 된다.
- 예시

  ```javascript
  // 💩 함수에 문자열 리터럴을 건네는 건 악취로 봐야한다!!!
  ...
  /** 생성자를 팰터리 함수로 바꾸기 👍 **/
  const creactEngineer = (name, type) => {
...
- 반대 리팩터링으로는 위에서 사용했던 `함수를 명령으로 바꾸기`이다.
- 간단하게 요약하자만 로직이 크게 복잡하지 않다면 명령 객체를 만드는 것보다는 그냥 평범한 함수를 사용하는게 좋다.
- 모든지 과하면 독이 된다. 간단한 로직은 일반 함수를 사용하고 명령 변경은 언어에 따라 다르지만 일급 함수를 사용해주자.
- 예시는 생략한다.

### 수정된 값 반환하기

- 데이터가 어떻게 수정되는지를 추적하는 일은 코드에서 이해하기 가장 어려운 부분 중 하나이다.
  - 특히 같은 데이터블록을 읽고 수정하는 코드가 여러곳이라면 ... 데이터 수정되는 흐름과 코드의 흐름을 일치 시키는것은 상당히 어렵다.
- 데이터가 수정된다면 그 사실을 명확히 알려주어서, 어느 함수가 무슨 일을 하는지 쉽게 알 수 있게 하는것이 대단히 중요하다.
- 수정됨을 알려주는 방법
  - 변수를 갱신하려는 함수라면 수정된 값을 반환하여 호출자가 그 값을 변수에 담아 두도록 하는 방법
    - 변수의 값을 단 한번만 정하면 될 때 특히 유용하다.
- 해당 리팩터링은 값 하나를 계싼한다면 분명한 목적이 있는 함수들에게 가장 효과적이고 반대로 값 어려개를 갱신하는 함수에는 효과적이지 않다.
- 예시
  ...
  function calculateAscent() {
    for (let i = 1; i < points.length; i++) {
      ...
      // 💩 해당 함수 내부르 보지 않으면  totalAscent값을 갱신한다는 것을 알 수 없다.
...
- 오류 코드를 연쇄적으로 전파하던 긴 코드를 예외로 바꿔 깔끔하게 제거할 수 있게 되었지만 모든지 과용하면 좋지 않다.
  - 예외 처리는 정망 말그래도 예외적으로 동작할 때만 쓰여야한다.
  - 함수 수행 시 문제가 될 수 있는 `조건`을 **함수 호출전에 검사할 수 있다면 예외처리가 필요없다!!!**
- 예시 코드는 생략 - 간단하게 에러가 생길 곳을 안다면 그전에 조건문으로 방어하면 된다!!

## 상속 다루기

- 객체 지향 프로그래밍에서 가장 유명한 특성인 상속이다.
- 이 역시 아주 유용함과 동시에 오용하기 쉬우니 조심하자!

### 메서드 올리기

- 중복 코드 제거는 중요하다.
  - 중복된 메서드는 당장 문제가 없을지 몰라도 미래에 한쪽이 변경되고 다른쪽이 변경되는 문제가 분명 발생할 수 있다 ..
    - 찾기 또한 엄청 힘들다..
- 메서드 올리기가 가장 쉬운 상황
  - 메서드들의 본문 코드가 똑같을 때이다. 이럴 떈 그냥 복사 붙여넣기하면 끝이다.
  - 해당 방법의 전제 조건은 테스트 코드가 얼마나 잘 만들었냐이기도 하자 .. 항상 테스트 코드는 잘 작성해두자
- 메서드 올리기를 적용하기 위한 선행 단계를 거쳐할 경우가 많고 서로 다른 두 클래스의 메서드를 각각 매개변수화하면 궁극적으로 같은 메서드가 되기도 한다.
  - 해당 경우에는 각각의 함수를 매개변수화 한다음 메서드를 상속 계층의 위로 올리면 된다.
...
- 반대 리팰터링 - `메서드 올리기`
- 특정 서브클래스에서 하나 혹은 소수와만 관련된 메서드가 있다면 굳이 상속을 해줄 필요 없이 상속을 제거하고 해당 메서드를 서브 클래스로 추가하는 편이 훨씬 깔끔하고 이해하기 쉽다.

### 필드 내리기

- 반대 리팰터링 - `필드 올리기`
- 해당 리팰터링도 서브클래서에서 하나 혹은 소수만 사용하는 필드라면 상속이 아닌 해당 Class에서 직접 사용하자

### 타입 코드를 서브클래스로 바꾸기

- 비슷한 대상들을 특정 특성에 따라서 구분해 줘야하는 경우가 자주 있다.
  - 이러한 문제르 해결하는 수단으로 타입코드 필드가 있다.
    - 해당 코드는 심볼, 문자열, 숫자, 열거형 등등으로 정의하여 사용 가능하다.
- 이러한 방법을 서브클래스에서도 사용가능한데 두 가지 면에서 특히 매력적이다.
  - 조건에 따라 다르게 동작하도록 다형성을 제공한다.
    - 코드 타이벵따라 동작이 달라져야하는 함수가 여러개일 때 특히 유용하다.
  - 특정 타입에서만 의미가 있는 값을 사용하는 필드나 메서드가 있을 때 발현된다.
  - 서브클래스 방식을 사용하면 관계를 더욱 명확히 드러내 준다.
- 예시
...
- 서브클래싱은 원래 데이터 구조와는 다른 변종을 만들거나 조율에 따라 동작이 달라지게 할 수 있는 유용한 메커니즘이다.
- 하지만 더 이상 쓰이지 않는 서브 클래스와 마주하는 프로그래머는 가치 없는 것을 이해하느라 에너지 낭비를 하게 되는 경우가 두루있다.

  - 이러한 경우 서브클래스를 슈퍼클래스의 필드로 대체해 제거하는것이 최선의 방법이다.

- 표현 방법을 바꾸려할 때 클라이언트 코드에 주는 영향을 최소화하는 방법은 생성자를 `팩터리 함수바꾸기`이다.
- 예시는 간단 설명 :: `타입 코드를 서브클래스로 바꾸기`와는 반대로 반대로 서브클래스에서 굳이 하는 일이 없다면 상속관계를 꺠버리자
...
- 비슷한 일을 수행하는 두 클래스가 보이면 상속 메커니즘을 이용해서 비슷한 부분을 공통으로 슈퍼 클래스로 옮겨 담아 주자
  - 공통된 데이터라면 : 필드 올리기
  - 공통된 동작이라면 : 메서드 올리기를 활용해주자
- 상속이라하면 현실세계에 활용하는 어떤 분류 체계와 같은 어려운 말로 설명 하지만 간단하게 말하자면 프로그램이 성장해 가면서 깨우치게 되며 슈퍼클래로  
  끌어올리고 싶은 공통 요소를 찾았을 떄 수행하는 사례가 잦았다.
- 슈퍼클래서 추출 하기의 대안이 있다.
  - 클래스 자체를 새로 만들어 버리는 `클래스 추춣하기`이다.
    이중 어느것을 선택하느냐는 중복 동작을 어떻게 처리하느냐애 따라 다를 뿐이다. 하지만 저자의 경우 `슈퍼클래스 추출`하기의 방법이 더 간단한 경우가 많았다 하였다.
  - 단 정답은 없으며 상황에 맞게 여러가지 방법으로 적용해 보는것아 중요하다.
- 예시

```javascript
class Employee{
  constructor(name,id, monthltCost){
    // this..~~ 코드
  }
  get name(){return this._name}
  get annualCost(){
    return this.monthltCost * 12;
  }
}

class Department{
  constructor(name,sfaff){
    // this..~~ 코드
  }
  get name(){return this._name}
  get annualCost(){
    return this.staff.map(e=>~~).reduce((sum,cost)=>sum+cost) * 12;
  }
}
...
// 💯 상단의 두 클래스에서 중복된 필드와 비슷한 동작을 해주는 매서드를 슈퍼클래스로 추출
class Party{
  constructor(name){
    this._name = name;
  }
...
- 계층구조를 리팩터링 하다 보면 기능들을 위로 올리거나, 내릴 경우가 많은데 이러한 과정을 거치다보면 하위클래스가 부모 클래스와 별반 차이가 없어지는 경우가 생긴다.
  - 이럴 경우에느 부모와 너무 같아져서 더는 독립적으로 존재할 필요가 없어지므로 해당 계층을 제거해 하나로 합치는게 좋다.

### 서브클래스르 위임으로 바꾸기

- 속한 갈래에 따라 동작이 달라지는 객체들은 상속으로 표현하는게 자연스럽다.
  - 공통 데이터와 동작은 모두 슈퍼클래스에 두고 서브클래스는 자신에 맞게 기능 추가 혹은 Override 시킨다.
- 하지만 상속에도 단점이 있다.
  - 가장 명확한 단점은 한번만 쓸 수 있는 카드라는 것이다.
    - 무언가가 달라져야 하는 이유가 여러개여도 상속에서는 그중 단 하나의 이유만 선택해 기준으로 삼을 수 밖에 없는 문제가 있다.
      - ex) 사람객체의 동작을 `나이대, 소득수준`에 따라 달리하고 싶아면 서버클래스는 젊은이 or 어르신 클래스 이거나 , 부자 or 서민 이어야 한다 둘다 갖을 수가 없어!!
    - 상속을 하게되면 클래스들의 관계를 아주 긴밀하게 결합하게된다. 따라서 부모를 수정하면 이미 존재하는 자식들의 기능에 영향을 줄 수 있기에 주의해야한다.
- 이러한 위에서의 두 문제는 위임을 사용해주면 모두 해결이 가능하다.
  - 위임은 클래스에 서로 다른 이유로 위임 할 수 있다. 위임은 객체사이의 일반적인 관계이므로 상호작용에 필요한 인터페이스를 명확히 정의 할 수 있다.
    - 따라서 상속보다 결합도가 훨씬 약하다는 큰 장점이 있다!!
- 하지만 여기서 포인트는 상속이 나쁘다는게 아니다 상속을 사용하다가도 위와 같은 문제가 발생한다면 서브클래스 위임으로 변경하면된다!!
- 예시는 복잡할 수 있으니 간단하게 설명 : 중간에 위임 클래스를 두고 해당 위임클래스를 이용해서 동작을 구분 및 실행 시키면된다.
  - 서브클래스가 다수일 경우에도 비슷하게 위임 클래스를 구분해서 생성할 수 있게 처리하면 된다.

### 슈퍼클래스를 위임으로 바꾸기

- 상속은 기존 긴으을 재활용하는 강력하고 손쉬운 수단이다.
- 하지만 상속이 혼란과 복작도를 키우는 방식으로 이뤄지기도 한다.
  - 대표적인 예로는 JAVA진영의 스택 클래스가 유명하다.
    - 리스트의 기능을 재활용 하려다가 스택에 필요없는 모든 기능까지 인터페이스에 노출되었었다
- 제대로 된 상속이라면 서브클래스가 슈퍼클래의 모든 긴으을 사용함은 물론이고 서브클래스의 인스턴스를 슈퍼클래의 인스턴스로도 취급할 수 있어야 한다.
- 위임을 사용하면 객체 분리를 쉽게 피할 수 있으며 기능 일부만 빌려올 뿐인, 서로 별개인 개념으로 명확해진다.
  - 하지만 위임에도 단점이 있다 위임 긴으을 이용할 호스트는 함수 모두를 전달함수로 만들어야하는 번거로움이 있다.
  - 이러한 일은 귀찮을 뿐 아주 단순해 문제가 생길 가능성이 적다는 장점이 있다.
- 이전 리팩터링에서도 항상 언급했듯 "상속은 절대 사용하면 안된다"가 아닌 상속만의 장점 또한 있다 `하위 모든 인스턴스가 상위 타입의 인스턴스가 가능하거나 의미상 적합한 조건이라면 상속은 간단하고 효과적인 메커니즘이다.` 그렇기에 상황에 맞춰서 리팩터링을 하는 것이 중요하다!
